...
export default function AksesMateri() {
  ...
  // 2. Ambil materi untuk kelas yang diikuti (KECUALI OBJEK 3D)
  useEffect(() => {
    ...
    onSnapshot(qMateri, (snapshot) => {
      ...
      // FILTER: Sembunyikan 'Model 3D (.glb)' agar hanya muncul dokumen/video biasa
      const filteredMateri = allMateri.filter(m => m.tipe !== 'Model 3D (.glb)');
      setMateriKelas(filteredMateri);
    });
  }, [kelasDiikuti]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400">Pilih materi dokumen/video dari kelas yang Anda ikuti.</p>
      </div>
      ...
      {materiKelas.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center dark:border-slate-700 dark:bg-slate-800">
          ...
          <h3 className="text-xl font-bold text-slate-700 dark:text-white">Belum Ada Materi Dokumen</h3>
          <p className="text-slate-500">Guru Anda belum mengunggah materi teks/video untuk kelas ini.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {materiKelas.map((materi) => (
            <div key={materi.id} className="flex flex-col justify-between rounded-2xl bg-white p-6 shadow-sm border border-slate-100 dark:bg-slate-800 dark:border-slate-700 hover:border-blue-300 transition-colors">
              ...
              {/* TOMBOL BUKA MATERI BIASA (Karena 3D sudah dipisah ke menu lain) */}
              <a 
                href={materi.link} 
                target="_blank" 
                rel="noopener noreferrer"
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-bold text-white transition hover:bg-blue-700 mt-4"
              >
